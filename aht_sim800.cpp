...
char AHT_SIM800::sendSMS(const char* phone, const char* content)
{
    ...
    if(sendAndCheckReply("AT+CSCS=\"UCS2\"", "OK", 2500) != AT_REPLY_FOUND)
    {
        return 0;
    }
    ...
    printPhone(phone);
    DB_Println("phone");
    if(readResponse(5000, ">") == AT_REPLY_FOUND)
    {
        printlnSMS(content);
        _uart->write(0x1A);
...
uint8_t AHT_SIM800::readPhoneBook(PHONE_BOOK* phoneBook, uint8_t from, uint8_t to)
{
    char at[15];
    sprintf(at, "AT+CPBR=%d,%d", from, to);
    DB_Println(at);
    _uart->println(at);
    ...
    {
        segmentRetVal = readSegment("OK", 2000);
        if(segmentRetVal == SEG_END && strstr(_buffer, "+CPBR:"))
        {
            char* p1 = NULL;
            char* p2 = NULL;

            p1 = strchr((char*)_buffer, ':');
            if (p1 == nullptr)
            {
                continue;
            }
            *(p1 + 3) = '\0';
            ...
            phoneBook[index].index = atoi(p1 + 1);
            DB_Print("index: ");
            DB_Println(phoneBook[index].index);
            ...
            *p2 = '\0';
            strcpy(phoneBook[index].phone, p1 + 1);
            DB_Print("phone: ");
            DB_Println(phoneBook[index].phone);
            ...
            *p2 = '\0';
            strcpy(phoneBook[index].name, p1 + 1);
            DB_Print("name: ");
            DB_Println(phoneBook[index].name);
